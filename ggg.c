...
Program *prog)
{
	...
	printf("set%2d [label = \"{globals|functions}\"];\n", setid );
...
void p_VarList( VarList *locals, int nest )
{
	...
	while( locals ) {
		v = locals->var;
		printf( "|{%s|%x|%d|%d|%d|%x}",
				v->name, v->ty, v->is_local, v->offset, v->is_static, v->initializer );
...
void p_Type( Type *T, int dummy ) {
	...
	while( T ) {
		printf("setType%02d [ label = \"{<H%02d> kind|size|align|is_incomplete|array_len|base}|{%s|%d|%d|%d|%d|<B%02d>%x}\"];\n",
				setidType,
				setidType,
				pType(T->kind),
				T->size,
		...
		if( T->base ) {
			printf("setType%02d:B%02d -> setType%02d:H%02d;\n",
					setidType, setidType, setidType+1, setidType+1);
		}
		setidType++;
		T = T->base;
	}
